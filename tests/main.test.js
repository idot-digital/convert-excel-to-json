...
describe('Execution via CLI', function() {
	...
	it('should work when passing in a JSON to --config', async function() {
		const stdout = await execCli(`${cliPath}  --config='{"sourceFile": "${sourceFileTest}"}' `);
		const result = stdout.replace(RegExp(os.EOL, 'g'), '');
		assert.equal(result, expectedResult);
	});

	it('should work when passing a file path to --sourceFile', async function() {
		const stdout = await execCli(`${cliPath}  --sourceFile=${sourceFileTest}`);
		const result = stdout.replace(RegExp(os.EOL, 'g'), '');
		assert.equal(result, expectedResult);
	});
});
